	...
	var BarChart = (function (_React$Component) {
	  ...
	  _createClass(BarChart, [{
	    ...
	    value: function _renderGraph(props) {
	      ...
	      var svg = d3.select(DOMNode).attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom);
	    ...
	    value: function _reusableGraph(props) {
	      ...
	      d3.select('.graph').selectAll('.bar').data(props.data).enter().append('rect').on('click', this._handleBarClick.bind(this)).attr('class', 'bar').attr('x', function (d) {
